...
# Importar modelos
try:
    from models.betting import User, Bet, Transaction, PlatformStats
except ImportError:
    # Definir modelos inline para Railway
    ...
    class Bet(db.Model):
        ...
        status = db.Column(db.String(20), default='open')  # open, matched, in_progress, completed, cancelled
    ...
    class Transaction(db.Model):
        ...
        type = db.Column(db.String(20), nullable=False)  # deposit, withdrawal, bet_placed, bet_won, platform_fee
        amount = db.Column(db.Numeric(10, 2), nullable=False)
    ...
    class PlatformStats(db.Model):
        ...
        total_volume = db.Column(db.Numeric(12, 2), default=0.00)
        platform_revenue = db.Column(db.Numeric(10, 2), default=0.00)
...
def create_user():
    try:
        ...
        user = User(
            email=data['email'],
            name=data['name'],
            password_hash=data['password'],  # Em produção, usar hash
